...
@router.get("")
async def list_api_keys(user_id: str = Depends(require_user_id)):
...
@router.post("")
async def create_api_key(req: CreateKeyRequest, user_id: str = Depends(require_user_id)):
...
async def delete_api_key(key_id: str, user_id: str = Depends(require_user_id)):
    supabase = get_supabase()
