...
import { BlastScientificPlots } from './BlastScientificPlots';
import { ReferenceComparisonWorkbench } from './ReferenceComparisonWorkbench';
...
export function BlastPanel({ hits, count, source, querySequence, fullSequences }: BlastPanelProps) {
  ...
  return (
    <div className="space-y-4">
      <motion.div variants={fadeUp} whileHover={cardHover} className="data-card overflow-hidden">
        ...
        <motion.div variants={stagger} className="divide-y divide-glass-border">
          {safeHits.length === 0 ? (
            ...
            return (
              <motion.div key={`${hit.accession}-${i}`} variants={fadeUp}>
                <button
      ...
      <BlastScientificPlots hits={safeHits} />

      {safeHits.length > 0 && (
        <ReferenceComparisonWorkbench
          analysisType="blast"
          bionexusResult={comparisonPayload}
          title="Compare this BLAST run with NCBI BLAST"
          defaultTopN={Math.min(10, safeHits.length)}
        />
      )}
    </div>
  );
