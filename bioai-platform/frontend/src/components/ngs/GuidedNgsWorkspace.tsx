...
const DNA_STEPS: PipelineStep[] = [
  ...
  },
  {
    ...
    short: 'Raw QC',
    description: 'Measure Phred base quality, GC, adapters, duplication and ambiguous bases before trimming. FASTQ base quality is not MAPQ: it describes confidence in each base call, not confidence in genomic placement.',
  ...
  },
  {
    ...
    short: 'Align',
    description: 'Resolve the declared genome build before mapping. The preview uses an explicit seed-and-extend surrogate; production uses the pinned Sarek mapper/index path. Mapping location is an inference, and reference-build compatibility is part of provenance.',
  ...
  },
  {
    ...
    short: 'BAM QC',
    description: 'Inspect SAM/BAM alignment evidence: FLAG state, CIGAR operations, mapping rate, MAPQ, duplication and—only when mate-aware alignment was actually evaluated—proper-pair and insert-size metrics.',
...
const RNA_BLUEPRINT: PipelineStep[] = [
  ...
  {
    id: 'rna-design', title: 'Audit design & replication', short: 'Design',
    description: 'Confirm biological replication and experimental units, inspect recorded batch/run/lane variables, reject perfect confounding or a rank-deficient model, and preserve the declared contrast before statistical fitting.',
    productionTools: 'Metadata audit + R model.matrix / DESeq2 design', stageKeys: [], output: 'design_audit.json + replicate counts + full-rank evidence',
  },
  {
