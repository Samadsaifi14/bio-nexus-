...
type Stage = { step?: string; tool?: string; version?: string; inputs?: string[]; outputs?: string[]; qc?: { status?: string; decision?: string; metrics?: Metric[] } | null; decision?: string; data?: Record<string, unknown> };
...
export default function StageEvidenceTable({ stages = [] }: { stages?: Stage[] }) {
  if (!Array.isArray(stages) || stages.length === 0) return <div className="rounded-xl border border-glass-border bg-surface-1 p-4 text-sm text-text-muted">No QC stages were returned by the backend for this run.</div>;
      ...
      <tbody className="divide-y divide-glass-border">
        {stages.map((stage, index) => {
          ...
          return <tr key={`${stage.step ?? 'stage'}-${index}`} className="align-top hover:bg-surface-1/60">
            ...
            <td className="px-3 py-3"><div className="font-mono text-text-primary">{stage.tool ?? 'Tool not reported'}</div><div className="mt-0.5 text-[10px] text-text-muted">{stage.version || 'version not reported'}</div></td>
