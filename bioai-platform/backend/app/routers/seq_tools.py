...
from app.tools.motif_scanner import MotifError, MOTIF_LIBRARY, scan_library, scan_pattern
...
async def scan_motif_library(req: LibraryScanRequest):
    try:
        return scan_library(req.sequence)
    except MotifError as e:
        raise HTTPException(status_code=400, detail=str(e))
...
async def list_motif_patterns():
    ...
    return [{"name": name, "description": entry["description"], "pattern": entry["pattern"]}
...
class DotPlotRequest(BaseModel):
    ...
    stringency: int = Field(80, ge=1, le=100, description="Percent identity required in the window")
...
async def run_dotplot(req: DotPlotRequest):
    try:
        return compute_dotplot(req.seq_a, req.seq_b, window=req.window, stringency=req.stringency)
