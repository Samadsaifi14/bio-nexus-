...
import { ScientificDataViews } from '@/components/results/ScientificDataViews';
...
function numericTokens(value: string): NumericToken[] {
  return (normalizeText(value).match(/-?\d+(?:\.\d+)?(?:e[+-]?\d+)?%?/gi) ?? []).map(text => ({ text, value: Math.abs(parseFloat(text.replace(/%$/, ''))) }));
}
...
export function AIResultSummary({ toolName, result, title='AI Evidence Engine', compact=false, jobId }: AIResultSummaryProps) {
  ...
  const [traces,setTraces]=useState<Record<string,EvidenceClaimTrace>>({});
  ...
  return <div className="space-y-4">
    <ScientificDataViews toolName={toolName} result={result} />

    <motion.div variants={fadeUp} className="rounded-2xl border border-glass-border bg-surface-0 p-5">
      <div className="mb-3 flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-2"><Brain className="h-5 w-5 text-accent-cyan"/><div><h3 className="font-semibold text-text-primary">{title}</h3><p className="mt-0.5 flex items-center gap-1 text-[10px] text-text-muted"><ShieldCheck className="h-3 w-3"/>Numeric claims are checked against the emitted result; persisted experiment claims can expose their evidence chain.</p></div></div>
        {!interpretation&&!loading&&<motion.button variants={fadeIn} onClick={handleInterpret} className="rounded-lg border border-accent-cyan/25 bg-accent-cyan/8 px-4 py-2 text-sm font-medium text-accent-cyan transition hover:bg-accent-cyan/12" disabled={loading}>Explain result</motion.button>}
      </div>
