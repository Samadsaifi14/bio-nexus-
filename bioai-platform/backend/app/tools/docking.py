...
# ── synchronous entry point (for thread-pool worker; avoids asyncio.run in thread) ───

def run_docking_sync(input: dict) -> dict:
    """Synchronous docking — uses subprocess.run + httpx.Client (no asyncio)."""
    ...
    try:
        ...
        try:
            ...
            with _httpx.Client(timeout=30) as client:
                r = client.get(pdb_url)
                if r.status_code != 200:
                    return {"error": f"PDB not found at {pdb_url}"}
                pdb_content = r.text
        except Exception as exc:
            return {"error": f"Failed to fetch PDB: {exc}"}
        ...
        r_prot = subprocess.run([OBABEL_CMD, clean_path, "-O", protein_pdbqt, "-xr"],
                             capture_output=True, timeout=120)
        if r_prot.returncode != 0 or not os.path.exists(protein_pdbqt):
            err = r_prot.stderr.decode() if r_prot.stderr else "obabel failed"
            return {"error": f"Protein PDBQT preparation failed: {err}"}

        # 4. Convert SMILES to 3D PDBQT
        ligand_pdbqt = os.path.join(tmpdir, "ligand.pdbqt")
        if not OBABEL_CMD:
            OBABEL_CMD = _pip_install_obabel()
        if not OBABEL_CMD:
            return {"error": "Open Babel not available – cannot prepare ligand PDBQT."}
        r_lig = subprocess.run([OBABEL_CMD, f"-:{smiles}", "-O", ligand_pdbqt, "--gen3d", "-h"],
                            capture_output=True, timeout=120)
        if r_lig.returncode != 0 or not os.path.exists(ligand_pdbqt):
            err = r_lig.stderr.decode() if r_lig.stderr else "obabel failed"
        ...
        try:
            ...
            stdout = r_vina.stdout
            stderr = r_vina.stderr
        except subprocess.TimeoutExpired:
            return {"error": "Docking timed out after 10 minutes"}

        if r_vina.returncode != 0 or not os.path.exists(out_pdbqt):
            err = stderr.decode("utf-8", errors="replace")[:500] if stderr else ""
            return {"error": f"Vina failed (exit {r_vina.returncode}): {err}"}
        ...
        # 8. Interaction fingerprinting on best pose
        ...
        # 9. Per-pose interaction summary
