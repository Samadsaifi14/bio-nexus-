"""Machine-auditable multi-assay NGS stage orchestrator."""
...
from app.ngs.contracts import Decision, QcStatus, StageContract, StageResult, run_contract

logger = logging.getLogger(__name__)

BENCHMARK_REGISTRY = [
    {
        "id": "giab-hg002-v4.2.1", "source": "NIST Genome in a Bottle",
        ...
        "url": "https://www.nist.gov/programs-projects/genome-bottle", "status": "NOT_EVALUATED", "metrics": None,
        "reason": "This run did not supply HG002 reads, matching truth VCF/benchmark BED, and a GA4GH comparison report.",
    },
    {
        "id": "precisionfda-truth-v2", "source": "FDA precisionFDA Truth Challenge V2",
        ...
        "url": "https://precision.fda.gov/challenges/10/results", "status": "NOT_EVALUATED", "metrics": None,
        "reason": "No challenge-compatible callset and stratified evaluation output were produced by this run.",
    },
    {
        "id": "seqc-gse47774", "source": "NCBI GEO / SEQC consortium",
        ...
        "url": "https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE47774", "status": "NOT_EVALUATED", "metrics": None,
        "reason": "No production RNA-seq quantification and SEQC comparison report were imported for this preview.",
    },
]

PRODUCTION_REQUIREMENTS = [
    {"id": "complete_input", "label": "Complete FASTQ ingestion", "required": True, "detail": "All records processed; no preview cap or silent subsampling."},
    {"id": "production_alignment", "label": "Production read alignment", "required": True, "detail": "Executed BWA-MEM2/DRAGMAP or another validated aligner with command and version."},
    {"id": "bam_artifacts", "label": "Indexed alignment artifacts", "required": True, "detail": "Coordinate-sorted BAM/CRAM, index, flagstat, idxstats and alignment metrics."},
    {"id": "recalibration", "label": "Reference-matched recalibration", "required": True, "detail": "BQSR or a documented no-BQSR workflow using build-matched known sites."},
    {"id": "production_calls", "label": "Production germline callset", "required": True, "detail": "Caller-generated VCF/gVCF with genotype, DP, GQ, AD, filters and index."},
    {"id": "sample_qc", "label": "Sample QC and identity", "required": True, "detail": "Coverage, duplication, insert size, contamination, sex and concordance when applicable."},
    {"id": "truth_benchmark", "label": "GIAB/GA4GH truth comparison", "required": True, "detail": "hap.py/vcfeval precision, recall and F1 inside the matching benchmark BED, stratified by SNP/INDEL."},
    {"id": "reproducibility", "label": "Reproducible execution record", "required": True, "detail": "Reference/resource checksums, exact commands, pipeline revision and container digests."},
]

RNASEQ_PRODUCTION_REQUIREMENTS = [
    {"id": "complete_input", "label": "Complete FASTQ ingestion", "required": True, "detail": "All RNA-seq FASTQ records processed without preview truncation."},
    {"id": "raw_qc", "label": "FastQC and MultiQC", "required": True, "detail": "Per-sample read QC plus cohort-level MultiQC artifacts."},
    {"id": "splice_alignment", "label": "Splice-aware alignment", "required": True, "detail": "Pinned STAR/HISAT2-compatible workflow with BAM/index and alignment QC."},
    {"id": "quantification", "label": "Gene/transcript quantification", "required": True, "detail": "Salmon or RSEM abundance outputs and merged count/TPM matrices where applicable."},
    {"id": "annotation_basis", "label": "Reference annotation provenance", "required": True, "detail": "FASTA/GTF versions and checksums, with index provenance."},
    {"id": "expression_qc", "label": "Expression-level QC", "required": True, "detail": "Assignment/biotype QC and sample-level PCA/heatmap or equivalent QC evidence."},
    {"id": "seqc_benchmark", "label": "SEQC/external benchmark when validation is claimed", "required": True, "detail": "External truth/reference comparison with predefined acceptance thresholds."},
    {"id": "reproducibility", "label": "Reproducible execution record", "required": True, "detail": "nf-core/rnaseq revision, exact argv, container digests, input/reference checksums and execution trace."},
]
...
class Pipeline:
    ...
    def run(self, sample: dict) -> dict:
        for contract in self.stages:
            self.state["pipeline_report"] = {"stages": [r.to_dict() for r in self.results]}
            ...
            if stage_result.qc:
                ...
                if unevaluated:
                    self.warnings.append(f"[{contract.step}] Not evaluated: {unevaluated}.")
                    ...
                    for metric in stage_result.qc.metrics:
                        ...
                        self.warnings.append(f"[{contract.step}] {metric.name}: {metric.value}{expected}{detail}")
            if stage_result.decision == Decision.STOP:
                self.stopped_at = contract.step
                logger.warning("Pipeline '%s' stopped at %s (blocking QC FAIL)", self.name, contract.step)
                break
    ...
    def _build_provenance(self, sample: dict) -> dict:
        ...
        return {
            ...
            "analysis": {
                "assay": sample.get("assay"), "sample_type": sample.get("sample_type"),
                "platform": metadata.get("platform"),
                "demonstration_data": bool(sample.get("demonstration_data") or metadata.get("demonstration_data")),
                "synthetic_reference": bool(sample.get("synthetic_reference")),
            },
            "inputs": files,
            "reference": reference,
            "tools": [
                {"stage": r.step, "implementation": r.tool, "version": r.version, "evidence_level": r.evidence_level}
                for r in self.results
            ],
        }

    def _production_requirements(self) -> list[dict]:
        source = RNASEQ_PRODUCTION_REQUIREMENTS if self.name == "rna-seq" else PRODUCTION_REQUIREMENTS
        return [{**item, "status": "MISSING", "evidence": None} for item in source]
    ...
    def report(self) -> dict:
        ...
        relevant_benchmarks = [b for b in BENCHMARK_REGISTRY if (b["id"].startswith("seqc") if is_rna else not b["id"].startswith("seqc"))]
        return {
            "pipeline": self.name,
            ...
            "validation": {
                ...
                "production_requirements": self._production_requirements(),
...
def wgs_wes_germline_stages(include: Optional[list[str]] = None) -> list[StageContract]:
    ...
    all_stages = {
        ...
        "structural_variant": stage15_contract(), "copy_number": stage16_contract(), "annotation": stage17_contract(),
        "knowledge": stage18_contract(), "prioritization": stage19_contract(), "final_gate": stage21_contract(),
    }
    if include:
