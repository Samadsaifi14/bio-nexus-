...
SPECS = [
    ...
    *[BenchmarkSpec(f"NGS-{name.upper().replace('_','-')}", "ngs", name, metric, direction, acceptance)
    ...
    BenchmarkSpec("AI-CITATION", "ai", "citation_fidelity", "higher", "every citation identifier resolves to supplied evidence"),
...
def numeric_fidelity(generated_text: str, evidence_text: str) -> dict:
    """Exact-token numeric fidelity; conservative by design.

    Derived arithmetic should be supplied as explicit evidence before this
    evaluator is used.  A number absent from evidence is therefore unsupported.
    """
...
def unsupported_claim_rate(claims: list[dict]) -> dict:
    """Evaluate structured Evidence-Aware AI claims.

    A claim is supported only when it carries at least one evidence reference
    and its evidence class is not ``unsupported/insufficient evidence``.
    """
    total = len(claims)
    ...
    return {
        ...
        "passed": math.isclose(rate, 0.0),
    }


def evaluate_ai_bundle(bundle: dict) -> dict:
    numeric = numeric_fidelity(bundle.get("generated_text", ""), bundle.get("evidence_text", ""))
    citations = citation_fidelity(bundle.get("generated_citations") or [], bundle.get("allowed_citations") or [])
    claims = unsupported_claim_rate(bundle.get("claims") or [])
    return {
        "suite": "BBS-2",
        "benchmarks": {"numeric_fidelity": numeric, "citation_fidelity": citations, "unsupported_claim_rate": claims},
        "passed": numeric["passed"] and citations["passed"] and claims["passed"],
    }
