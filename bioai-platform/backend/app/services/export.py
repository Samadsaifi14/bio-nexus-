...
def export_blast_pdf(result: dict, sequence: str = "") -> bytes:
    ...
    elements.append(Paragraph(f"<b>Provider:</b> {result.get('source') or 'not recorded'}", body_style))
    elements.append(Paragraph(f"<b>Requested database:</b> {result.get('requested_database') or result.get('database') or 'not recorded'}", body_style))
    elements.append(Paragraph(f"<b>Executed database:</b> {result.get('executed_database') or result.get('database') or 'not recorded'}", body_style))
    elements.append(Spacer(1, 12))
    ...
    for i, hit in enumerate(hits[:20], 1):
        ...
        table_data.append([
            ...
            f"{hit['query_coverage_pct']}%" if hit.get('query_coverage_pct') is not None else "not available",
        ])
    ...
    elements.append(Paragraph("E-values depend on the searched database, program and search space. Evaluate each match with its bitscore, identity, query coverage and the cutoff selected for this analysis.", body_style))
