...
function excluded(url: string): boolean {
  return EXCLUDED_URL_PARTS.some(part => url.includes(part));
}
...
function tables(v: unknown, prefix = 'result', depth = 0, out: TableModel[] = []) {
  ...
  for (const [k, child] of Object.entries(v)) {
    const path = `${prefix}.${k}`;
    if (Array.isArray(child) || obj(child)) tables(child, path, depth + 1, out);
  }
  return out;
}
  ...
  return (
    <div className="rounded-xl border border-glass-border bg-surface-1 p-4">
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="text-sm font-semibold text-text-primary">{human(s.yKey)} vs {human(s.xKey)}</p>
          <p className="break-all font-mono text-[10px] text-text-muted">{s.path}</p>
        </div>
        <span className="text-[10px] text-text-muted">{pts.length} points</span>
      </div>
      <svg viewBox={`0 0 ${W} ${H}`} className="mt-2 w-full" role="img" aria-label={`${human(s.yKey)} versus ${human(s.xKey)}`}>
        {tick.map(t => {
          const y = T + t * (H - T - B);
          const val = maxY - t * dy;
          return <g key={`y${t}`}><line x1={L} x2={W - R} y1={y} y2={y} stroke="currentColor" className="text-white/5"/><text x={L - 7} y={y + 4} textAnchor="end" fontSize="9" fill="currentColor" className="text-text-muted">{fmt(val)}</text></g>;
        })}
        {tick.map(t => {
          const x = L + t * (W - L - R);
          const val = minX + t * dx;
          return <g key={`x${t}`}><line x1={x} x2={x} y1={T} y2={H - B} stroke="currentColor" className="text-white/5"/><text x={x} y={H - 20} textAnchor="middle" fontSize="9" fill="currentColor" className="text-text-muted">{fmt(val)}</text></g>;
        })}
...
function ResultTable({ t }: { t: TableModel }) {
  ...
  return (
    <details className="overflow-hidden rounded-xl border border-glass-border bg-surface-1" open={t.rows.length <= 35}>
      <summary className="cursor-pointer px-4 py-3 text-sm font-semibold text-text-primary">
        {human(t.path.split('.').pop() || t.path)} <span className="ml-2 text-xs font-normal text-text-muted">{t.rows.length} rows</span>
      </summary>
      <div className="overflow-x-auto border-t border-glass-border">
        <table className="min-w-full text-xs">
          <thead><tr>{t.columns.map(c => <th key={c} className="whitespace-nowrap px-3 py-2 text-left text-text-secondary">{human(c)}</th>)}</tr></thead>
          <tbody className="divide-y divide-glass-border">
            {rows.map((r, i) => <tr key={i}>{t.columns.map(c => <td key={c} className="max-w-[420px] whitespace-pre-wrap break-words px-3 py-2 align-top font-mono text-text-primary">{fmt(r[c])}</td>)}</tr>)}
          </tbody>
        </table>
      </div>
...
function PayloadPanel({ capture }: { capture: Capture }) {
  ...
  const raw = useMemo(() => {
    try {
      return JSON.stringify(capture.payload, null, 2);
    } catch {
      return String(capture.payload);
    }
  }, [capture.payload]);
    ...
    <div className="space-y-4">
      <div className="data-card overflow-hidden">
        <div className="flex flex-wrap items-start justify-between gap-3 border-b border-glass-border p-4">
          <div>
            <p className="text-sm font-semibold text-text-primary">{human(capture.key.split('/').filter(Boolean).pop() || 'Scientific response')}</p>
            <p className="max-w-3xl break-all font-mono text-[10px] text-text-muted">{capture.url}</p>
          </div>
          <span className="text-[10px] text-text-muted">{capture.at}</span>
        </div>
...
export function ScientificDataSurface() {
  ...
  useEffect(() => {
    ...
    const patchedFetch: typeof window.fetch = async (...args) => {
      ...
      try {
        ...
        if (requestUrl.includes('/api/backend') && ct.includes('application/json')) {
          void response.clone().json().then(data => commit(requestUrl, data)).catch(() => undefined);
        }
      } catch {
        // Observational only; never interfere with the scientific request.
      }
      return response;
    };
    ...
    const patchedOpen = function(this: XMLHttpRequest, method: string, url: string | URL, ...rest: unknown[]): void {
      ...
      Reflect.apply(originalOpen as unknown as (...args: unknown[]) => unknown, this, [method, url, ...rest]);
    ...
    const patchedSend = function(this: XMLHttpRequest, body?: Document | XMLHttpRequestBodyInit | null): void {
      ...
      Reflect.apply(originalSend as unknown as (...args: unknown[]) => unknown, this, [body]);
  ...
  return createPortal(
    <section className="mt-6 space-y-5" data-scientific-data-surface="true">
      <div className="data-card overflow-hidden">
        <div className="flex flex-wrap items-start justify-between gap-3 border-b border-glass-border p-5">
