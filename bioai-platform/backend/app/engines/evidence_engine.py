...
class EvidenceEngine(BaseEngine):
    ...
    tool = "Evidence Graph"
    tool_version = None
    databases = ["derived from provenance graph"]  # no single reference database
    parameters = {
        ...
        "confidence": "high (2+ sources) / medium (1) / low (0)",
    ...
    def validate(self, result: EngineResult) -> ValidationReport:
        ...
        checks.extend([
            ...
            {
                "name": "claim_fields",
                "passed": all(c.get("text") and c.get("confidence") in CONFIDENCE_TIERS for c in claims),
    ...
    def _export_csv(self, result: EngineResult) -> str:
        graph = result.evidence.get("graph") or {}
        rows = [
            "claim_id,claim,confidence,evidence,rejected",
        ]
    ...
    def figure(self, result: EngineResult) -> str:
        ...
        for s in sources:
            color = "#2563eb"
            parts.append(
                f'<rect x="{x}" y="{y}" width="240" height="26" rx="4" fill="{color}" opacity="0.85"/>'
        ...
        for c in claims:
            ...
            y += 18
        parts.append(
            f'<text x="{x}" y="{y + 6}" font-size="10" fill="#6b7280">Generated by BioNexus Evidence Engine v{self.version}</text>'
        )
        parts.append(
            '<style>svg { background: #ffffff; } rect { stroke: none; }</style>'
        )
        return (
