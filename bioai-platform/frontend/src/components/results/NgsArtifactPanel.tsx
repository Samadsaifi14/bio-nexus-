...
type Artifact = { name: string; filename: string; mime: string; content: string; description: string; kind: 'vcf' | 'sam' | 'json' };
...
function VcfPreview({ text }: { text: string }) {
  const rows = useMemo(() => text.split(/\r?\n/).filter(l => l && !l.startsWith('##')).slice(0, 30), [text]);
  const header = rows.find(l => l.startsWith('#CHROM'))?.replace(/^#/, '').split('\t') ?? [];
  const data = rows.filter(l => !l.startsWith('#')).map(l => l.split('\t'));
...
export default function NgsArtifactPanel({ vcf, sam, pipeline }: { vcf?: string; sam?: string; pipeline: unknown }) {
  const artifacts: Artifact[] = [
    ...(vcf ? [{ name: 'Variant calls', filename: 'bionexus-variants.vcf', mime: 'text/plain', content: vcf, description: 'VCF records generated by the variant-calling pipeline.', kind: 'vcf' as const }] : []),
    ...(sam ? [{ name: 'Alignments', filename: 'bionexus-alignments.sam', mime: 'text/plain', content: sam, description: 'SAM alignment records used by downstream QC and visualization.', kind: 'sam' as const }] : []),
    { name: 'Pipeline report', filename: 'bionexus-pipeline-report.json', mime: 'application/json', content: JSON.stringify(pipeline, null, 2), description: 'Complete machine-readable audit report with stages, QC metrics, decisions and provenance.', kind: 'json' as const },
  ];

  return <div className="space-y-4">
    <div className="grid gap-3 md:grid-cols-3">{artifacts.map(a => <button key={a.filename} onClick={() => downloadText(a.filename, a.content, a.mime)} className="rounded-xl border border-glass-border bg-surface-1 p-4 text-left transition hover:border-accent-cyan/35"><div className="flex items-center justify-between"><div className="flex items-center gap-2 text-sm font-semibold text-text-primary">{a.kind === 'vcf' ? <Dna/> : a.kind === 'sam' ? <Table/> : <FileText/>}{a.name}</div><DownloadSimple className="text-accent-cyan"/></div><p className="mt-2 text-[11px] leading-4 text-text-muted">{a.description}</p><p className="mt-3 font-mono text-[10px] text-accent-cyan">Download {a.filename}</p></button>)}</div>
    {vcf && <section><h3 className="mb-2 text-xs font-semibold uppercase tracking-wider text-text-muted">VCF preview</h3><VcfPreview text={vcf}/></section>}
    {sam && <section><h3 className="mb-2 text-xs font-semibold uppercase tracking-wider text-text-muted">SAM preview</h3><SamPreview text={sam}/></section>}
    <details className="rounded-xl border border-glass-border"><summary className="cursor-pointer px-4 py-3 text-xs font-medium text-text-secondary">Advanced: machine-readable pipeline JSON</summary><pre className="max-h-80 overflow-auto border-t border-glass-border bg-surface-1 p-4 text-[10px] leading-5 text-text-muted">{JSON.stringify(pipeline, null, 2)}</pre></details>
  </div>;
}
